...
from keras.layers import Input, Conv2D, MaxPooling2D, Permute, Reshape, add, Dense, Dropout, Flatten, RepeatVector, \
    Permute, multiply, Lambda, concatenate, Activation
...
cwt_txt1 = './dataset/train15-3.txt'

cwt_path2 = './dataset/cwt/'
cwt_txt2 = './dataset/val15-3.txt'

cwt_path3 = './dataset/cwt/'
cwt_txt3 = './dataset/test15-3.txt'

x_train_savepath = './dataset/15-3/x_train.npy'
y1_train_savepath = './dataset/15-3/y1_train.npy'
y2_train_savepath = './dataset/15-3/y2_train.npy'

x_test_savepath = './dataset/15-3/x_test.npy'
y1_test_savepath = './dataset/15-3/y1_test.npy'
y2_test_savepath = './dataset/15-3/y2_test.npy'

delta_t_train_savepath = './dataset/15-3/delta_t_train.npy'
delta_t_test_savepath = './dataset/15-3/delta_t_test.npy'

x_test_savepathc = './dataset/15-3/x_testc.npy'
y1_test_savepathc = './dataset/15-3/y1_testc.npy'
y2_test_savepathc = './dataset/15-3/y2_testc.npy'
delta_t_test_savepathc = './dataset/15-3/delta_t_testc.npy'
...
if os.path.exists(x_train_savepath) and os.path.exists(y1_train_savepath) and os.path.exists(
        y2_train_savepath) and os.path.exists(
        x_test_savepath) and os.path.exists(y1_test_savepath) and os.path.exists(y2_test_savepath) and os.path.exists(
    delta_t_train_savepath) and os.path.exists(delta_t_test_savepath):
    print('-------------Load Datasets-----------------')
...
history = model.fit(x_train, y_true_train, batch_size=32, epochs=300, validation_data=(x_val, y_true_val),
                    validation_freq=1)
model_json = model.to_json()
with open("./result/15-3/cnn/cwt_model.json", 'w') as json_file:
    json_file.write(model_json)
model.save_weights('./result/15-3/cnn/cwt_model.h')
model.summary()
with open("./result/15-3/cnn/cwt.txt", 'w') as f1:
...
json_file = open("./result/15-3/cnn/model.json")
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("./result/15-3/cnn/model.h")
