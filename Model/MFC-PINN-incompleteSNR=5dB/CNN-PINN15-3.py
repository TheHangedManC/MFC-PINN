...
x = Dense(4096, activation='relu')(input_layer1)
x1 = Dense(4096, activation='relu')(x)
input_layer2 = Input(shape=IMAGE_DIMS)
conv1 = Conv2D(32, (3, 3), activation='relu', padding='same')(input_layer2)
...
x2 = Dense(4096, activation='relu')(dropout1)
x = concatenate([x1, x2])
x = Dense(4096, activation='relu')(x)

output = Dense(2, activation='linear')(x)  # Output both x and y coordinates
model = Model(inputs=[input_layer1, input_layer2], outputs=output, name='cnn-pinn_model')
...
model.compile(optimizer=Adam(lr=0.001), loss=custom_loss, metrics=[mse])

history = model.fit([delta_t_train, x_train], y_true_train, epochs=300, batch_size=32, validation_data=([delta_t_val, x_val], y_true_val), validation_freq=1)

model_json = model.to_json()
with open("./result/15-3/cnn-pinn/model.json", 'w') as json_file:
    json_file.write(model_json)
model.save_weights('./result/15-3/cnn-pinn/model.h')
# model.summary()
with open("./result/15-3/cnn-pinn/loss.txt", 'w') as f1:
...
json_file = open("./result/15-3/cnn-pinn/model.json")
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("./result/15-3/cnn-pinn/model.h")
