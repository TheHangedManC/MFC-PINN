...
def generateds(path, txt):
    ...
    x1, x2, y1_1, y1_2 = train_test_split(x, y1_, test_size=0.2, random_state=789)
    x1, x2, y2_1, y2_2 = train_test_split(x, y2_, test_size=0.2, random_state=789)
    delta_t1, delta_t2, y1_1, y1_2 = train_test_split(delta_t, y1_, test_size=0.2, random_state=789)
...
pinn_model = Model(inputs=[input_layer1, input_layer2], outputs=output, name='cnn-pinn_model')
...
history = pinn_model.fit([delta_t_train, x_train], y_true_train, epochs=300, batch_size=32, validation_data=([delta_t_val, x_val], y_true_val), validation_freq=1)


model_json = pinn_model.to_json()
with open("./result/15-3/cnn-pinn/model.json", 'w') as json_file:
    json_file.write(model_json)
pinn_model.save_weights('./result/15-3/cnn-pinn/model.h')
pinn_model.summary()
