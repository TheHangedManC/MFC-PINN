...
model = build_trilateration_model()
model.compile(optimizer=Adam(lr=0.001), loss=custom_loss, metrics=[mse])
history = model.fit(delta_t_train, y_true_train, epochs=300, batch_size=32, validation_data=(delta_t_val, y_true_val), validation_freq=1)

model_json = model.to_json()
with open("./result/15-3/pinn/model.json", 'w') as json_file:
    json_file.write(model_json)
model.save_weights('./result/15-3/pinn/model.h')
# model.summary()
with open("./result/15-3/pinn/loss.txt", 'w') as f1:
...
json_file = open("./result/15-3/pinn/model.json")
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("./result/15-3/pinn/model.h")
